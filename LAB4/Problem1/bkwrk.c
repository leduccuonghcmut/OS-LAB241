...
int bkwrk_create_worker() {
  ...
  for (i = 0; i < MAX_WORKER; i++) {
    ...
    if (pid == -1) {
      // Lỗi xảy ra khi tạo worker process
      perror("fork");
      return -1;
    } else if (pid == 0) {
      // Quá trình con - worker process
      bkwrk_worker((void *)i);
      exit(0); // Kết thúc worker process sau khi hoàn thành công việc
    }
#endif
...
int bkwrk_dispatch_worker(unsigned int wrkid) {
  ...
  if (kill(pid, SIG_DISPATCH) == -1) {
    // Lỗi xảy ra khi gửi tín hiệu đến worker process
    perror("kill");
    return -1;
  }
#endif
